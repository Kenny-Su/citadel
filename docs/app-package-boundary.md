...
- Snake is the current external-app proof: root build scripts install it from a packed artifact instead of workspace build/watch maintenance; tests also pack it through `scripts/pack-workspace-app.mjs`, install the tarball through npm into a temp host with no workspaces and an empty `workspace-apps.json`, generate a Snake-only installed-app catalog, and boot the host/server path from that packed dependency shape.
...
- `local-external-apps.json` declares local source packages that should be consumed as packed installed artifacts during the migration; currently that pilot is Snake.
